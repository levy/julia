                                   ...
                                   uint32_t **atomicfields_out, uint32_t **constfields_out)
{
    ...
    if (validate) {
        for (size_t i = 0; i + 1 < jl_svec_len(fattrs); i += 2) {
            ...
            jl_value_t *attr = jl_svecref(fattrs, i + 1);
...
static jl_value_t *resolve_type_refs(jl_value_t *t, htable_t *subst_map, jl_deferred_typecache_t *dcache)
{
    ...
    if (jl_is_typeapp(t)) {
        ...
        if (cached != HT_NOTFOUND)
            return cached;
        // Collect params from the nested TypeApp chain
        // TypeApp(TypeApp(head, p1), p2) => head, [p1, p2]
        jl_value_t *head = t;
        size_t n = 0;
        while (jl_is_typeapp(head)) {
            n++;
            head = ((jl_typeapp_t*)head)->head;
        }
        JL_GC_PUSH1(&head);
        jl_value_t **resolved;
        JL_GC_PUSHARGS(resolved, n);
        // Walk the chain again to collect and resolve params
        jl_value_t *cur = t;
        ...
        jl_value_t *result;
        // Special case: Union is stored as TypeApp with head == Union
...
JL_DLLEXPORT jl_value_t *jl_resolve_typegroup(jl_module_t *module, jl_svec_t *typevars, jl_svec_t *struct_infos, jl_svec_t *old_types)
{
    ...
    { JL_TRY {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            jl_sym_t *name = tv->name;

            // Read struct info fields:
            ...
            JL_GC_PUSH3(&tv, &info, &fieldnames);
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (datatypes[i]->name->wrapper == NULL) {
                ...
                JL_GC_PUSH3(&tv, &params, &wrapper);
                jl_setup_type_wrapper(datatypes[i]->name, params, &wrapper);
                results[i] = wrapper;
                // Update substitution map to use wrapper (UnionAll) for parametric types
                ptrhash_put(&subst_map, tv, results[i]);
                JL_GC_POP();
            }
            else {
                JL_GC_PUSH1(&tv);
    ...
    { JL_TRY {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (!mutabl && dt->types != NULL) {
                ...
                if (n == 1) {
                    // Single-struct group (ordinary struct definitions): only
                    // self-references are possible, so mirror `_typebody!`
                    // exactly. references_name has the precise affects_layout
                    // handling required for e.g. circular type parameters that
                    // do not affect the layout (issue #33954).
                    ...
                    // Mutually recursive group: references_name only follows
                    // type parameters, so it misses cycles that go through the
                    // fields of other group members. Use the conservative
                    // field-following reachability check (cycle-safe via its
                    // visited set) instead.
                    htable_t visited;
