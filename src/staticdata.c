...
{
    // A region open during image output would write region-tagged pages
    // and region cursors into the image, where they have no meaning on
    // load; refuse, rather than emit a corrupt image. The window closes at
    // its @with_region boundary, so this is a usage error, not a limit.
    extern int jl_gc_region_current(void);
