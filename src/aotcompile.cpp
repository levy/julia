        ...
        jl_emission_params_t *params)
{
    ...
    uint64_t t_shards = 0;
    // The optimization level of a delta's code (`--reactive-delta-opt`): a
    // rebuild can trade the speed of the edited cone for its compile time.
    if (reactive_format && jl_reactive_reuse_enabled()) {
        int delta_opt = jl_reactive_delta_opt();
        if (delta_opt >= 0)
            jl_options.opt_level = delta_opt;
    }
    ...
    if (params->emit_metadata) {
        ...
        uint64_t t_metadata = jl_hrtime();
        if (jl_reactive_timings() && reactive_format)
            jl_safe_printf("reactive: dump shards %.2f s (%u threads, opt %d)\n", (t_metadata - t_shards) / 1e9, threads, jl_options.opt_level);
        metadata_outputs = compile(metadataM, "data", 1, [](Module &) {});
        if (jl_reactive_timings() && reactive_format)
            jl_safe_printf("reactive: dump metadata %.2f s\n", (jl_hrtime() - t_metadata) / 1e9);
    }
...
#define WRITE_ARCHIVE(fname, field, prefix, suffix) \
    if (fname) {\
        ...
        uint64_t t_archive = jl_hrtime();
        WRITE_ARCHIVE(obj_fname, obj, "", ".o");
        if (jl_reactive_timings() && reactive_format)
            jl_safe_printf("reactive: dump archive %.2f s\n", (jl_hrtime() - t_archive) / 1e9);
