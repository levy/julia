...
SHARE="$($SEALED_HOST_JULIA --startup-file=no -e 'print(joinpath(Sys.BINDIR, "..", "share", "julia"))')"
...
setup)
    ...
    $SEALED_HOST_JULIA --startup-file=no --project="$HERE/env2" -e 'using Pkg; Pkg.instantiate()'
    ;;
build)
    mkdir -p "$HERE/work2"
    $SEALED_HOST_JULIA --startup-file=no "$HERE/probe.jl" source abstract 6 > "$HERE/work2/abstract6.jl"
    $SEALED_HOST_JULIA --startup-file=no --project="$HERE/env2" "$SEALED_JULIAC" \
...
compare)
    mkdir -p "$HERE/work3"
    $SEALED_HOST_JULIA --startup-file=no "$HERE/probe.jl" source abstract 6 > "$HERE/work3/abstract6.jl"
    $SEALED_HOST_JULIA --startup-file=no "$SHARE/juliac/juliac.jl" \
