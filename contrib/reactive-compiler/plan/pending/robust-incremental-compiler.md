...
Stages 0 to G are built and gated: the oracle, the rebuild without
execution, the ledger and the classifier, the fresh table and the dead
code, the server, the trimmed product (under overlays too, with the memo
of its pass), the image by pages and the overlay image. A routing edit
rebuilds in 2.1 to 2.3 s through the server, the binary starts in 0.23 s,
and the omnet front end drives it with one bare command
(`plan/done/reactive-build-interface.md` of omnet-julia). What stays open:

- the invalidation itself: closed by measurement (2026-09-09, the log
  entry of that day). A redefinition of the hub method caps one code
  instance and no caller, and its delta holds 37 functions; the 981
  functions of the Stage G record were the harness itself, which every
  delta carried while it lived in `Main`, and H1 moved it into the system
  image. The hub edit rebuilds in 0.8 s, as a leaf edit does. What stays
  is the floor of the machinery, and the size of an overlay: its full
  relocation lists, 14.7 MB of its 22 MB.
- the trim-clean flagship: the routing sample has 315 unresolved call
  sites for the stock verifier; the trimmed product of the flagship and
  the measurable gain of the memo wait for the sealed-abstract merge.
- Stage H, the generalization, is built (2026-09-09): the server is a
  runtime entry, the variables are flags, and `julia -m PackageCompiler`
  founds, rebuilds, watches, reports and stops a store; the omnet front
  end is a client with its catalog.
- Beyond: the source diff for the packages of the Manifest, and the
  compaction of the global slots without a founding.
...
| Keyword | Variable | Flag of `julia` (Stage H) | Values | Default | Stage |
| --- | --- | --- | --- | --- | --- |
| `server` | `JULIA_REACTIVE_SERVER` | `--reactive-server=<socket>` starts one | `false`, `true` | `false` | D, built |
...
| `trim` | `JULIA_REACTIVE_TRIM` | `--trim=safe` of the fork; `--reactive-trim-memo=<file>` | `:off`, `:on`, `:once` | `:off` | E |
| `image` | `JULIA_REACTIVE_IMAGE_WRITE` | `--reactive-image=whole\|pages\|overlay` | `:whole`, `:pages`, `:overlay` | `:whole` | F, G |
| `delta_opt` | `JULIA_REACTIVE_DELTA_OPT` | `--reactive-delta-opt=N` | 0 to 3, -1 the build's | -1 | G, built |
| `compact` | `JULIA_REACTIVE_COMPACT` | — | `(saves = N, growth = f)` | `(50, 0.25)` | F |
| `founding` | `JULIA_REACTIVE_FOUNDING` | — | `false`, `true` | `false` | F |
| — | `JULIA_REACTIVE_REUSE` | `--reactive-reuse` | 0, 1 | 0 | built |
| — | `JULIA_REACTIVE_IMAGE` | `--reactive-image-format` | 0, 1 | 0 | built |
| — | `JULIA_REACTIVE_TIMINGS` | `--reactive-timings=N` | 0, 1, 2 | 0 | built |
| — | `JULIA_REACTIVE_HEAPDUMP` | `--reactive-heapdump=<file>` | a path | unset | built |
| — | `JULIA_REACTIVE_DIRTY_PAGES` | `--reactive-dirty-pages=<file>` | a path | unset | F, built |

A flag sets a field of `jl_options`; an unset field reads the variable, so
a gate drives a build through the environment and a flag wins. The
keywords of `materialize_app` and of the omnet front end pass flags to
the children they start; the variables without a flag (`SERVER`,
`COMPACT`, `FOUNDING`, `TRIM`) are the builder's own.
...
- [ ] the scan of the invalid code instances that no statement names: not
      built, and superseded (2026-09-09): Stage B's ledger works that seam
      and the oracle accepts every stage; the scan stays out.
      Observed on routing: the child reports "cone 0 replaced, 0
...
- [x] the option `trim` (`:off`, `:on`, `:once`) and its variable; the
      ...
      *As built (2026-09-08):* `trim::Symbol` and `JULIA_REACTIVE_TRIM`
      ...
      ones as hard links.
- [x] the trimmed product under overlays (Stage G left it out): a
      trimmed write is a whole write from the heap of an overlay-loaded
      process. The texts of an overlay cannot join a whole link: they
      were compiled against the overlay's own slot table (`gvar_base` and
      `shard_base` 0) and their shard symbols collide with the founding's.
      So the trimmed product reuses the founding image's functions only,
      by name in the founding's texts, and compiles the functions of the
      overlays again from their IR in the heap, with the delta's cone;
      the compaction bounds that cost. In the runtime: a set
      `jl_options.trim` turns the page and overlay modes off for the
      writer and the emitter (the loader's state stays); the chain loader
      records the origin image of every composed id, and
      `jl_reactive_image_ids` answers 0 for a function of an overlay
      under trim, so the compile pass serves it from its IR. In the
      builder: the link takes the founding's texts alone; the overlay of
      a save joins the chain only after the trimmed product is written,
      so a refusal leaves the chain and both bundles as they were. Gate E
      with `IMAGE=overlay`.
      *As built (2026-09-09, a83259c6c7 + 843c307ac9; the builder in
      package-compiler-reactive):* as designed. Gate E with
      `IMAGE=overlay` passes on TrimApp: the founding and six server edits
      print both sums, the trimmed images stay within 12 KB of each other
      (4.04 MB against 167 MB), the child build after the chain prints
      its sums (4.79 MB, the residue as in whole mode), the dynamic edit
      is refused with T1 and the base, the overlays, the chain and the
      trimmed image are as they were. `gate_e.sh` takes `IMAGE` and
      `TRIM`, and its refusal check hashes the chain too. The server's
      trim fork reports "28 of 37204 functions reused, bases gvar 9334
      shard 8": the base's slots and shards, as a whole write needs.
- [x] the incremental verification: the reachable set of a save is the
      ...
      the routing sample. First the measurement of that pass on the
      routing store (the sample is not trim-clean, but the pass runs to
      its refusal and reports its time).
      *Measured (2026-09-09):* on the routing store (overlay chain, the
      server's fork, the entry point `@ccallable`) the trim request takes
      4.6 s to its refusal, against 2.4 s for the untrimmed save; the
      verifier names 315 unresolved call sites, `getproperty` on untyped
      fields of `OmnetRunner` and `iterate` over configuration entries,
      so the routing sample stays not trim-clean until the sealed-abstract
      compiler is merged. On TrimApp the same pass is 1.6 to 1.7 s with
      14 to 18 served code instances and 540 compiled fresh: the code
      instances of the image mostly hold no inferred IR (a system image
      keeps the IR of inlineable code only), so the trim pass infers the
      reachable program again from source, and the verification is the
      smaller part. The incremental form is therefore a memo, written by
      a successful trim fork beside the store and read by the next: for
      every served code instance, its image id, its verified callees as
      ids, and the flag that the verifier accepted it; the next pass
      serves such a code instance without inference or IR, enqueues its
      callees through a map from image id to code instance built once
      per fork from the method tables, and verifies only what the memo
      does not hold. A code instance that an edit invalidated is not
      reusable and is not in the served set, so its replacement is walked
      fresh, and the memo stays correct. Not built: without a trim-clean
      program of a real size the memo's effect cannot be shown (TrimApp
      serves 14 code instances), and a refused pass writes no memo. It
      waits for the sealed-abstract merge, with the flagship as its
      measure.
      *Built (2026-09-09, 504b564f3e + bca774c4ff, against TrimApp as the
      user asked):* the memo as designed, `<store>/trim-memo.txt` by
      name, read and written by the runtime (`jl_reactive_trim_memo_read`
      / `_write`, the ids of this process), used by `compile!` under trim:
      a served code instance in the memo is taken with its callees and
      without a compile; the IR-served and the fresh-compiled paths record
      their entries (a target without an image id, or an entry point or
      finalizer it enqueues, leaves the entry out). The map from id to
      code instance is emptied before the write: as a global of the
      Compiler it kept every code instance of the image in the trimmed
      heap (18 MB instead of 4). The counts are printed by the runtime,
      because a forked writer exits without a flush of the Julia streams.
      Gate E, whole and overlay: the founding's pass walks 199 code
      instances and records them; every edit's pass serves all 199 from
      the memo and compiles 153 code instances fresh instead of 557
      (the entry points, the juliac patches and the hooks, which are not
      in the image); the products print the sums, the dynamic edit is
      refused, the memo-served images are flat (3.76 MB, 253 KB below the
      founding's fresh compile, so the size invariant compares the edits
      among themselves). The front stays at 1.5 s on TrimApp: its cost
      there is the fixed part, not the walk; the routing-sized saving
      waits for a trim-clean flagship.
...
- [x] the writer: the bytes of a clean page come from the base image's
...
- [x] the format of an overlay: the new objects with relocations into the
...
- [x] the builder: no link; the bundle gains one shared object per save;
      ...
      *As built (2026-09-08):* `_reactive_overlay_link` links the save's
      ...
      for the child. The trimmed product under overlays is the Stage E
      item below (2026-09-09). Found with it: a no-server child on a
      chain crashed in the overlay emit, because the baseline of an
      object of an overlay was read from the base file past its extent
      (the overlay write sets the merged extent on the base's record);
      the bound is the base file's own extent now, and such an object
      has no baseline, a whole patch. Gate G ran no child build after
      the word-run cut; Gate E with `IMAGE=overlay` does.
...
image any more. TrimApp: a server rebuild in 0.5 s (heap 0.2 s).

*The second, taken apart (2026-09-08, the second run):* the dirty-page
report of a founding-started server names `jl_method_table_activate` as
the writer of 2290 pages: the invalidation of the edit's cone stores one
word, `max_world`, into 91 000 code instances, and every page of them
went whole into the overlay (20 MB). Four cuts, each a commit:
- an overlay patches objects, not pages, and only the words that differ
  from their baseline (the last chain patch of the object, else the
  base file): the sysimg patches are 915 KB instead of 20 MB;
- `JULIA_REACTIVE_DELTA_OPT` sets the optimization level of a delta's
  code (the omnet builder compiles at 3): at level 1 the shards take
  0.37 s instead of 0.58 s, and every hop check still passes;
- the server keeps the ledger of its last apply as the next apply's old
  ledger: the ledger costs 0.12 s instead of 0.28 s;
- the pre-queue of a save walks the dirty pages, not every object.
The rebuild is 2.1 to 2.2 s at level 1 (2.4 to 2.6 at level 3). What
stays is the edit's cone: the front's inference 0.5 s, the emission
0.2 s, the shards 0.4 s, the heap 0.6 s (queue 0.2, write 0.2, combine
0.1), the apply 0.25 s. A hub method like `handle_message!` invalidates
981 functions and 91 000 code instances; an edit of a leaf method has a
cone of a few functions, and its rebuild is the machinery's floor, about
1 s (a fresh server: front 0.2 s, heap 0.3 s). The bound of Gate G holds
for small cones and not for a hub; a smaller cone per edit is a change
of the invalidation, beyond this plan.

## Stage H — the generalization

The reactive compiler's interface is the server's protocol, and today its
only client is PackageCompiler, which also carries the harness the server
runs. The omnet front end (`plan/done/reactive-build-interface.md` of
omnet-julia) put a face on it: the bare command rebuilds once a store
exists, `--status`, `--stop-server`, `--dev`, `--watch`, `--trim`. Part of
that face is generic. This stage moves the generic part down: the server
into the runtime, the variables into flags, the verbs into a
PackageCompiler command line. The omnet front end keeps its catalog of
builds and `BUILD_NAMES`, its app module writer with the usage and the
build info, `--distribution`, and its model, workload and backend options,
and becomes a client. The server goes first: as a runtime entry, any
build tool can drive it, juliac and Pkg's precompile included, and
PackageCompiler is one client among them.

- [x] H1, the server as a runtime entry. `julia --reactive-server=<socket>
      --sysimage=<image> --project=<dir>` starts the loop that
      `reactive_server.jl` of PackageCompiler runs today (234 lines; the
      requests `apply <spec.toml>`, `save <archive>`, `trim <archive>
      [dir]`, `status`, `quit`, one line each way, the reply `ok`,
      `refused <file>` or `error <message>`). The harness the loop runs
      moves into the Julia tree as the stdlib `ReactiveCompiler`, built
      into the system image like `Compiler`, so that every image the
      server loads holds it and the start includes nothing:
      `reactive_child.jl` (1111 lines: the ledger, the classifier, the
      apply, the trace precompile), `SourceDiff.jl` (473 lines, vendored
      twice today) and `trim_entrypoints.jl`. PackageCompiler's copies go;
      `_reactive_server_ensure` starts `julia --reactive-server`, and the
      no-server child runs the module's `apply` and `save` from a
      three-line script. The protocol is the interface and does not
      change; `tool/server_request.py` stays the reference client. A
      trimmed product does not carry the module, because the trim's walk
      does not reach it. Decisions at the start: the name of the stdlib;
      whether the fork stays in `libjulia-codegen` (`jl_reactive_fork`) or
      the module reaches it through the runtime; the world in which the
      module's methods run inside an app image (the server evaluates the
      edits in the app's world, as the script does today).
      *As built (2026-09-09, 9c32c24ff8, 997be08e90, ea4dd73c7b; the
      client 840b708 of package-compiler-reactive; the omnet front end
      2c2267a1):* the stdlib is `ReactiveCompiler`
      (`stdlib/ReactiveCompiler`, uuid `1dda119b-…`), in the system image
      through `base/sysimg.jl`, `stdlib/stdlib.mk` and the `STDLIBS` list of
      `stdlib/Makefile` (the symlink); its files are the harness's as they
      were (`child.jl`, `server.jl`, `entrypoints.jl` with
      `trim_entrypoints!()` in place of the included file, `SourceDiff.jl`),
      with `precompile` statements of the entries so the image holds their
      code. `--reactive-server=<socket>` is a field of `jl_options`
      (`jloptions.h`, `jloptions.c`, `base/options.jl`), and `exec_options`
      of `base/client.jl` requires the stdlib by uuid and calls `serve`,
      which sets the paths the image write emptied (`Sys.BINDIR`,
      `Sys.STDLIB`, `LOAD_PATH` from `--project`, the depot) and runs the
      loop; the pid file went. PackageCompiler starts `julia
      --reactive-server`, and its child, refresh and trim scripts call the
      stdlib by uuid; its four copies of the harness went. The three
      decisions: the name `ReactiveCompiler`; the fork stays in
      `libjulia-codegen`, called by the module; the world is the app's,
      as before. Found on the way: the storage of `jl_options` lives in
      the loader library (`cli/loader_lib.c`, `jl_exported_data.inc`),
      which `make` does not rebuild for a header change, so a new field
      reads garbage past it until `cli/*.c` are touched; the stdlib
      Makefile keeps its own `STDLIBS` list beside `stdlib.mk`; and the
      omnet front end passed the rebuild keywords explicitly, which hid
      the gates' variables (`JULIA_REACTIVE_FOUNDING`) — its defaults are
      the variables now. Gate E (whole and overlay) and Gate G pass
      through the runtime server, the routing rebuild at 1.3 to 1.4 s
      (2.1 before: the harness is compiled in the image); the face
      rebuilds an edit in 1.3 s.
- [x] H2, the variables as flags. `--reactive-reuse`,
      `--reactive-image=whole|pages|overlay`, `--reactive-delta-opt=N`,
      `--reactive-trim-memo=<path>`, `--reactive-timings=N`,
      `--reactive-heapdump=<path>`, `--reactive-dirty-pages=<path>` and
      `--reactive-image-format` become fields of `jl_options`
      (`jloptions.h`, `jloptions.c`), with the `JULIA_REACTIVE_*` variables
      as their defaults and the flag winning. A fork sets the fields it
      changes through a setter (`jl_reactive_set_trim`, one for the memo
      path) instead of the environment. Every predicate of `staticdata.c`
      and `aotcompile.cpp` reads the field. The option table of this plan
      names the flag beside the variable.
      *As built (2026-09-09, 599bc97f9a and the fixes to d42a74fbf7; the
      builder f778521):* eight fields of `jl_options`, unset as -1 or
      NULL, read through three accessors of `staticdata.c`
      (`reactive_option_string`, `reactive_option_level`,
      `reactive_image_write`); `jl_reactive_delta_opt()` replaces the
      emitter's own read; the fork names its memo with
      `jl_reactive_set_trim_memo`. PackageCompiler passes the flags to the
      rebuild child (in `sysimage_build_args`), the server, the founding's
      object process (`--reactive-image-format --reactive-image=whole`,
      so a gate's environment no longer has to be cleared) and the
      founding's trim child. Checked by hand: a flag alone, a variable
      alone, a flag over a variable, and two bad values refused. Found on
      the way, because the flags moved the last object of a chain: the
      window of an object rewritten in place ends within sixteen bytes
      before the next object, the writer's padding before one object, but
      at the end of an overlay the next object is the first of a later
      overlay and the gap is the padding of two sections — the last
      object of overlay 7 was refused with the first of overlay 8 sixteen
      bytes away. The chain loader records where every overlay's objects
      begin, and the window at such a boundary, as at the base's end and
      at the image's end, is bounded above alone. And the forked writer's
      report of an error compiled `showerror`, which the writer forbids
      once it started (`jl_type_infer` aborts), so the error was lost: the
      report is the message of an `ErrorException` or the type's name,
      written through the runtime. Gate E passes in both modes, Gate G
      passes, the omnet face founds and rebuilds with the flags on the
      server's command line, and `tool/gate_h.sh` passes with the flags
      on its hand-started server (its edit goes one above the increment
      of the last snapshot's copy of the file now).
- [x] H3, the PackageCompiler command line. `julia -m PackageCompiler
      <verb> <app_dir> [options]` with the verbs `build` (the founding when
      the output holds no store, else the rebuild: the `:auto` rule of the
      omnet face, which `create_app` takes as well), `status`, `stop` and
      `watch` (`reactive_watch` moves from omnet's `Reactive.jl` with its
      polling of the store's tracked list), and the options `--server`,
      `--image`, `--delta-opt`, `--found`, `--compact`, `--trim` as the
      keywords they set. `--dev` stays a policy of the front end. The omnet
      front end calls these and keeps its catalog.
      *As built (2026-09-09, package-compiler-reactive a0979a5; the omnet
      front end 82c91b26):* `src/reactive_cli.jl` with `@main`, so
      `julia -m PackageCompiler <verb> <app_dir> [options]` runs it; the
      verbs `build`, `status`, `stop`, `watch`; the options of a rebuild
      (`--no-server`, `--image=`, `--delta-opt=`, `--found`, `--compact=`,
      `--trim`) and of a founding (`--package=`, `--tracked=`,
      `--workload=`, `--executable=`, `--optimization=`, `--debug-info=`,
      `--cpu-target=`); a bad verb or option prints the usage. The include
      walk (`tracked_sources(package_dir)`, from the package's root file),
      the status printer (`print_store_status`) and the watch (`watch_app`)
      moved here from the omnet front end, which delegates and keeps its
      names; a founding takes the walk as its default tracked list, so
      `build --package=<dir>` needs no list. `create_app` takes
      `reactive = :auto`: a store under the app directory makes the build
      a rebuild, `true` founds one, `false` builds plain. Checked: the
      parser on every option and seven refusals; on the routing store
      `status`, `build` (a rebuild, 0.7 s), `watch` (a rebuild of 0.9 s on
      a save, Ctrl-C stops the watch and the server), `stop`; and TrimApp
      founded from `--package` and `--workload` alone in 61 s, rebuilt
      with `--delta-opt=1` in 0.6 s, its binary printing the sum of the
      edit. The omnet suite passes as before. `tool/gate_cli.sh` is the
      gate of the command line: on the routing store `status`, a rebuild
      (1.2 s), a watch that rebuilds on a save (1.5 s) and stops on Ctrl-C
      with its server, `stop`; then TrimApp founded from `--package` and
      `--workload` alone and rebuilt, its binary printing the sums. It
      passes (2026-09-09).
- [x] H4, the staged founding. A founding builds into `<app_dir>.founding`
      beside the app and swaps it in at the end (`<app_dir>.old` holds the
      previous app until the new one is whole): a build killed on the way
      leaves the old app and its store as they were, and a founding again
      no longer removes the old store first. The staging directory of a
      killed founding goes at the next founding. Gate: `tool/gate_cli.sh`
      founds TrimApp again, and founds it again once more with the build
      killed after its trace, after which the old store still rebuilds.
      *As built (2026-09-09, package-compiler-reactive cc2f081):* in
      `materialize_app`, for the first founding and a founding again
      alike; the old store is no longer removed before the build, its
      server is stopped. `tool/gate_cli.sh` passes: the founding again
      leaves no `.founding` or `.old`, one snapshot and the binary of the
      edit; a founding killed 25 s in (its log arrives late, Julia
      buffers stderr to a file, so the kill goes at a fixed delay) leaves
      the store, which rebuilds to s2 and prints the sum; the founding
      after that clears the staging directory.
- [x] Gate H. Gates D, E and G unchanged through the runtime server; and
      one rebuild of the routing sample driven by `server_request.py`
      alone — the server started by hand, `apply` and `save` sent by the
      script, the overlay linked by `_reactive_overlay_link` — as the
      proof that a client needs nothing of PackageCompiler but the link.
      *Second half passed (2026-09-09, `tool/gate_h.sh`):* the store of
      the omnet build copied, the server started by hand on the image of
      its last snapshot, a spec of the tracked files written by the
      script, `apply` ok, `save` ok in 0.7 s (a 25 MB archive), the
      overlay linked and put into the chain, and the binary's hop mean
      doubled with the edit.
      *First half passed (2026-09-09):* Gate E (whole and overlay) and
      Gate G pass unchanged; Gate D's checks pass — the hop means, the
      restart, the oracle, the start, the founding after the edits within
      72 bytes — and its rebuilds take 4.3 to 4.7 s, the floor of a whole
      write above the gate's 4 s bound, which pages and overlays meet, as
      the record of Stage D says.
...
- A founding again removes the old store before the new build, and
  `create_app` removes the app directory: a build killed during those
  minutes leaves no store (seen 2026-09-09, a Ctrl-C into a founding that
  the compaction rule had started under a watch). Every server
  generation appends its cumulative overlay to the chain, so a chain of
  four generations reaches the growth bound. The gates keep their
  rebuilds rebuilds with `--compact=1000,1.0`. Closed by H4: the founding
  is staged beside the app and swapped in at the end.

## Log

**2026-09-09, the cone.** The question of "the invalidation itself" was
measured before any design. A probe on the routing image redefines
`handle_message!(::AApp)` with one changed line under Julia's invalidation
log: the old method has one code instance, not inlineable, with return
type `Nothing`; the redefinition caps that one code instance and no
caller — the event loop calls the hub dynamically, and a call with more
methods than inference splits on records no edge — and the new method
infers to the same return type. Then the rebuild of that edit through the
server with the timings on: the delta holds 37 functions of 65 067, the
front takes 0.2 s, the heap 0.3 s, the save 0.6 s, `materialize_app`
0.8 s; a leaf edit is the same. The 981 functions and the 91 000 written
words of the Stage G record were the harness (`rc_*`, 1100 lines) that
every delta carried while it lived in `Main`, and the words that
`jl_method_table_activate` wrote for its methods; H1 put the harness into
the system image, and the cone of a hub edit is its own code instances.
The dirty pages of a save before the write are 541: locks taken and
released (`_jl_mutex_wait`, 76 pages), the tables of the trace and the
ledger (`ijl_array_grow_end`, `ijl_eqtable_put`), the remembered set of
the GC (`ijl_gc_queue_root`, 14). So the item closes without a change of
the invalidation. What stays: the floor of the machinery, and the size of
an overlay — its full merged relocation lists are 14.7 MB of its 22 MB,
and every server generation appends one, which is what reaches the
compaction bound after four. A lists delta per overlay is the next lever
on the size; the time is at its floor.

**2026-09-09, Stage H.** Stages 0 to G built, and the face of omnet-julia
showed which part of a front end is generic. The answer to "what of the
CLI generalizes into the modified Julia": the server's protocol is the
interface, so the server becomes a runtime entry first, the variables
flags, the verbs a PackageCompiler command line; the omnet front end keeps
its catalog. Bookkeeping: the boxes of the Stage F writer, the Stage G
format and the trimmed product under overlays flipped, all three built
with an "As built" note; Stage A's scan marked superseded by the ledger.
