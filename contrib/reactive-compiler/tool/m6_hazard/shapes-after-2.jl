# The call shapes across the reuse boundary, after the second edit: the
# text of `shapes-after.jl` with one changed line in `driver`. The oracle
# gate copies this file over `HazardApp/src/shapes.jl` after the first edit.
#
...
@noinline chained(x::Int) = x + CHAIN_BASE[] + 1
