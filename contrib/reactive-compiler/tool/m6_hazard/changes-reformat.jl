# The reformat of `changes-after.jl`: the same expressions with other
# comments, other blank lines and other spaces inside the lines. The rebuild
# of this text applies nothing; only the line numbers of the ledger move.
#
# (Gate B, H1: a reformat gives an empty apply set.)
...
abstract type Middle <: Kind2 end
...
for (name, k) in ((:times2, 2), (:times3, 30))
    @eval $name(x) = $k * x
end
use_times2() = times2(5)
use_times3() = times3(5)
