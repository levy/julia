# The stage-3 demo model. The checker's findings shape it: messages are
# pooled, because a message's lifetime is IN FLIGHT, not one event; the kept
...
# builds real temporary vectors and drops them. The point of stage 3 is that
# such code needs no hand-pooling: the Event region reset reclaims it.
...
function MiniKernel.handle_message!(net, m::Sink, env, gate)
    ...
    # pushes, which is the violation the stage-2 checker flagged.
