# Stage 4, the endurance run: does memory stay flat while the HIL runs?
#
#   ../../julia stage4_endurance.jl 18000000     # 30 min at 100 us
#
# The stage-3 model under regions, paced at one event per 100 us slot, with
...
# pages the reset returned, and the miss count. The claim under test: the
# high-water RSS stops growing after warmup, so the collector is never
# needed on the disciplined path, for arbitrarily long runs.
