# Stage 3, the proof run. Needs the patched build:
#
...
function main()
    ...
    network, sink = Stage3Model.build(relays, deliveries;
    ...
    elseif variant == "regions"
        ...
        # again in this process — reset is the region pages' only collector,
        # and the residual GC-interplay bug (see the plan) is v2 work. The
        # warm phase is small, so nothing needs collecting; the report path is
        # compiled here so nothing compiles after the measured phase either.
